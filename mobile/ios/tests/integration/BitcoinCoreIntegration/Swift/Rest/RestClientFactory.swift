...
class RestClientFactory : IRestClientFactory {
    ...
    func get(baseURL: URL) -> IRestClient {
        let client = DefaultRestClient(baseURL: baseURL)
        return client
    }
}
...
//
// Based on
//  https://gist.github.com/sarpsolakoglu/9033a319d5a199b4d702a094770f3afa
//
...
extension DefaultRestClient: SimpleRestClient {
    ...
    func performRequest<D: Decodable>(_ responseType: D.Type, request: URLRequest, completion: @escaping (D?, URLResponse?, Error?) -> Void) {
        self.session.dataTask(with: request) { (data, response, error) in
            if error != nil || data == nil {
                ...
                do {
                    let responseStr = String(data: responseData, encoding: String.Encoding.utf8)
